...
const PersonalReview = () => {
  ...
  return (
    <div>
        ...
        {
          revis.length === 0 && <p className="text-center my-10 text-4xl  bg-slate-100 p-8 shadow-xl">"you should <br /> give some <span className="text-teal-600 font-bold">Review"</span> </p>
        }
      

      {revis.map((rv) => (
        <div
          className=" photoURL w-10/12 mx-auto border border-slate-300 shadow-lg rounded-lg m-2 p-8 flex items-center justify-between" key={rv._id}>
          ...
          <div className="flex">
            ...
            <button className="btn btn-outline btn-error btn-sm ml-4 mb-2" onClick={()=>handleDelete(rv._id)}> <FaTrashAlt/> </button> <br />
            <button className="btn btn-outline btn-success btn-sm ml-4" > <FaEdit/> </button>
            </div>
        </div>
      ))}
    </div>
